...
export default function First() {
  ...
  return (
    <Wrapper>
      ...
      <ContentText>{`환율: ${Util.getBeautifiedNum(exchange.toFixed(2))} ${engCountry} / USD`}</ContentText>
      <InputFormWrapper>
        <SubTitle htmlFor='money'>송금액: </SubTitle>
        <InputMoney type='number' id='money' onChange={e => onChange(e, 'money')}></InputMoney>
        <ContentText>USD</ContentText>
        <button type='submit' onClick={moneySubmit}>Submit</button>
      </InputFormWrapper>
      {
        result <= 0 ? <></> : <ContentText>{`수취 금액은 ${result} ${engCountry} 입니다.`}</ContentText>
      }
    </Wrapper>
  );
}